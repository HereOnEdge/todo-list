...
export const creatTag = (tagName, tagColor) => {
  const newTag = __tag(tagName, tagColor);
  tags.push(newTag);
  return newTag;
};
creatTag("Work", "green");
...
export const tagHub = () => {
  ...
  const addFullContainer = (projectID, parentNode) => {
    ...
    for (let i = 0; i < tags.length; i++) {
      const tag = showTag(tags[i].name, projectID, false);
      ...
      tagContainer.addEventListener("click", () => {
        ...
        const choosedTagName = choosedTag.active;
  ...
  function showTagOptions(projectID) {
    const mainContainer = document.querySelector(
      `#tags-mainContainer-${projectID}`
    );
    const height = tags.length * 20 + 10;
    mainContainer.style.height = `${height}px`;
    const topPosition = 100 / tags.length;
    for (let i = 0; i < tags.length; i++) {
      const tagContainer = document.querySelector(
        `#tag-container-${i}-project-${projectID}`
      );
      setTimeout(() => {
      ...
      tagContainer.addEventListener("mouseenter", () => {
        setTimeout(() => {
          const idArray = tagContainer.id.split("-");
          const id = idArray[2];
          const tagName = document.querySelector(
            `#tagName-${id}-project${projectID}`
          );
          const tagNameStyles = getComputedStyle(tagName);
          tagName.style.color = tags[id].color;
          const width = tagNameStyles.width.split("px");
...
function __chooseTag(index, projectID) {
  const tagContainer = document.querySelector(
    `#tag-container-${index}-project-${projectID}`
  );
  ...
  if (activeTag) {
    ...
    activeTag.classList.remove("active");
    const activeFirstChild = activeTag.firstElementChild;
